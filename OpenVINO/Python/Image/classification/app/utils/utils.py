...
def process_help_inference_string(classification_app):
    output_string = 'Start inference {}ronously'.format(classification_app.api_type)
    if classification_app.api_type == 'async':
        output_string += ', {} inference requests'.format(classification_app.nireq)

        device_ss = ''
        if CPU_DEVICE_NAME in classification_app.device:
            device_ss += str(classification_app.ie.get_config(CPU_DEVICE_NAME, 'CPU_THROUGHPUT_STREAMS'))
            device_ss += ' streams for {}'.format(CPU_DEVICE_NAME)
        if GPU_DEVICE_NAME in classification_app.device:
            device_ss += ', ' if device_ss else ''
            device_ss += str(classification_app.ie.get_config(GPU_DEVICE_NAME, 'GPU_THROUGHPUT_STREAMS'))
    ...
    if classification_app.niter and not classification_app.duration_seconds:
        limits += '{} iterations'.format(classification_app.niter)
