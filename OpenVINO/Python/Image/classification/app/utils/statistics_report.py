...
class StatisticsReport:
    ...
    def dump(self):
        def dump_parameters(f, parameters):
            ...
            filename = 'classification_report_'+date+'.csv'
        with open(os.path.join(self.config.report_folder,filename), 'w') as f:
    ...
    def dump_performance_counters(self, perf_counts):
        ...
        if not perf_counts:
            ...
            return
        date = datetime.today().strftime('%d-%m-%Y-%H:%M:%S')
        filename = os.path.join(self.config.report_folder, 'classification_{}_report_{}.csv'.format(self.config.report_type,date))
