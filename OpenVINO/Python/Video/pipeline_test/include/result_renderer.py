...
FONT_SIZE = 0.7
TEXT_VERTICAL_INTERVAL = 35
TEXT_LEFT_MARGIN = 15


class ResultRenderer(object):
    def __init__(self, no_show, display_fps=True, display_confidence=True, number_of_predictions=1, labels=None,
                 output_height=720):
    ...
    def render_frame(self, frame, logits, timers, frame_ind):
        ...
        if logits is not None:
            ...
            print("Frame {}: {} - {:.2f}% -- {:.2f}ms".format(frame_ind, labels[0], (1-probs[0]) * 100, inference_time))
        else:
            labels = ['Preparing...']
            probs = [0.]
        ...
        for i, (label, prob) in enumerate(islice(zip(labels, probs), self.number_of_predictions)):
            display_text = text_template.format(label=label, conf=(1-prob) * 100)
        ...
        if not self.no_show:
            cv2.imshow("Earthquake Classification", frame)
...
def decode_output(probs, labels, top_k=None, label_postprocessing=None):
    ...
    #WORKS
    probs = top_ind
    #decoded_labels = [labels[0] if probs[0] < 0.5 else labels[1]]
    decoded_labels = [labels[0] if probs < 0.5 else labels[1]]
    #print("Decoded_labels: {}".format(decoded_labels))
