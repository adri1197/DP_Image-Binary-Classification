...
def run_pipeline(video,model,render_fn, fps=30):
    pipeline = AsyncPipeline()
    pipeline.add_step("Data", DataStep(video), parallel=False)
    pipeline.add_step("Data-Model", ModelStep(model), parallel=False)
    pipeline.add_step("Model", ModelInferenceStep(model), parallel=False)
...
class DataStep(PipelineStep):
    ...
    def process(self, item):
        ...
        self.n_frames = self.n_frames+1 if status == True else 0
        #print(self.n_frames)
        if not status:
            return Signal.STOP
        return frame
    ...
    def _open_video(self):
        ...
        print("Video: {} ({} frames)".format(next_video,int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT))))
...
class ModelStep(PipelineStep):
    ...
    def process(self, frame):
        ...
        return  {'encoder': self.own_time.last}, frame


class ModelInferenceStep(PipelineStep):

    def __init__(self, model, sequence_size=1, num_classes=2):
        super().__init__()
    ...
    def process(self, item):
        ...
        self._embeddings.append(frame)
        if len(self._embeddings) == self.sequence_size:
            model_input = np.concatenate(self._embeddings, axis=0)
            model_input = np.expand_dims(model_input, axis=0)

            logits, next_frame = self.async_model.infer(model_input, frame)
            if logits is None:
                return None
            #probs = softmax(logits - np.max(logits))
            #print(logits[0])

            return next_frame, logits[0], timers
