...
def main():
    ...
    for i in range(n):
        ...
        #plt.imshow(image)
        #plt.show()
        #print("Loaded image: {} ({})".format(os.listdir(args.input[0])[i],image.shape))
    ...
    for j in range(n):
        ...
        if os.path.isdir(args.input[0]): 
            print('Image: {} -> Prediction: {} ({})'.format(os.listdir(args.input[0])[j],results,categories[write_result]))
        else:
